...
class Data(threading.Thread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        custom_db_path = (
            getattr(builtins, "sc_data_db_path", None)
            or os.environ.get("SC_DATA_DB_PATH")
        )
    ...
    def _init_from_cache(self):
        ...
        try:
            if os.path.exists(self.cache_db_path) and os.path.exists(self.cache_hash_path):
                ...
                if cache_age < cache_ttl:
                    ...
                    if cached_hash:
                        ...
                        logger.debug("Using cached database (age: %.0fs, hash: %s)", cache_age, cached_hash)
                        return
                else:
                    logger.debug("Cached database is stale (age: %.0fs > TTL: %.0fs)", cache_age, cache_ttl)
    ...
    def _atomic_write_cache(self, content_source, db_hash):
        ...
        try:
            ...
            temp_db_fd, temp_db_path = tempfile.mkstemp(dir=self.cache_dir, suffix=".db.tmp")
            temp_hash_fd, temp_hash_path = tempfile.mkstemp(dir=self.cache_dir, suffix=".hash.tmp")
    ...
    def update(self):
        ...
        try:
            ...
            need_download = (
                remote_hash != self.actual_db_hash or
                remote_hash != cached_hash or
                cache_stale
            )
            ...
            logger.debug("Downloading new SQLite database (remote_hash=%s, cached_hash=%s, stale=%s)",
                        remote_hash, cached_hash, cache_stale)
    ...
    def run(self):
        ...
        while True:
            ...
            if success:
                ...
                logger.info("Retrying database update (attempt %d/%d) in %d seconds...", 
                           retry_count + 1, max_retries + 1, retry_delay)
                time.sleep(retry_delay)
                continue
            elif first_attempt:
                # After max retries, signal anyway to avoid blocking forever
                logger.warning("Failed to update database after %d attempts, using existing database", 
                              max_retries + 1)
                self.updated.set()
                first_attempt = False
